...
function searchnggallerytags($content) {
	...
	if ( stristr( $content, '[gallery' )) {
	  // added by hm for integration of wp-greetcard, 10.02.2008
	  $url_prefix="";
	  
	  $search = "@(?:<p>)*\s*\[gallery\s*=\s*(\w+|^\+)( url=(.*?))*\]\s*(?:</p>)*@i";
		
	  if	(preg_match_all($search, $content, $matches)) {
	    if (is_array($matches)) {
	  
	      // added by hm for integration of wp-greetcard, 10.02.2008
	      $url_prefix=substr($matches[2][0],5);
		...
		if($galleryID) {
		  $search = $matches[0][$key];
		  $replace= nggShowGallery($galleryID,$url_prefix); // added url_prefix as parameter, hm 10.02.2008
		  $content= str_replace ($search, $replace, $content);
		}
	      }	
	    }
...
/**********************************************************/
function nggShowGallery($galleryID, $url_prefix="") {
	...
	if (is_array($picturelist)) { 
	  $out = nggCreateGallery($picturelist,$galleryID,$url_prefix);
	}
...
/**********************************************************/
function nggCreateGallery($picturelist,$galleryID = false,$url_prefix="") {
	/** 
	* @array  	$picturelist
	* @int		$galleryID
    **/
	...
	foreach ($picturelist as $picture) {
		...
		// added by hm for integration of wp-greetcard, 10.02.2008
		if ($url_prefix!="") {
		  $link=stripslashes($url_prefix . "?gallery=" . $galleryID . "\&amp;image=" . $folder_url.$picture->filename);
		  $thumbcode = ""; // no effects for greetcards :-)
		} else
